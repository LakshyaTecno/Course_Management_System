...
const allowedCourseStatus =[constants.courseStatus.approved,
    constants.courseStatus.pending,
    constants.courseStatus.rejected,
  ] 


const newCourseBody = (req,res,next)=>{
    try{
        if (!req.body.title) {
        ...
        if (!req.body.posterUrls) {
            return res.status(400).send({
                message: "Failed ! Course posters are not provided"
            });
        }
    
        next();
    }catch{
        console.log("#### Error while velidating new Course request body ##### ", err.message);
        res.status(500).send({
...
const editCourseBody = (req,res,next)=>{
    try{
        ...
        if (req.body.courseStatus && !allowedCourseStatus.includes(req.body.courseStatus)){
            return res.status(400).send({
        ...
        console.log("#### Error while velidating edit course request body ##### ", err.message);
        res.status(500).send({
